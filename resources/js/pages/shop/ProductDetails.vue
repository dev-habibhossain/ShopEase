...
const reviews = ref<Review[]>([
    { name: 'Rina A.', rating: 5, date: '12 Jun 2026', verified: true, text: 'Sound quality is amazing and the noise cancellation actually works on the bus. Battery lasts me almost a week. Highly recommended!' },
    { name: 'Karim H.', rating: 5, date: '5 Jun 2026', verified: true, text: 'Delivered fast inside Dhaka, paid cash on delivery. Build quality feels premium for the price.' },
    { name: 'Sadia R.', rating: 4, date: '28 May 2026', verified: false, text: 'Very comfortable for long use. Only wish the case was a bit smaller, but overall great value.' }
]);
...
<template>
    ...
    <nav aria-label="Breadcrumb" class="border-b border-gray-200 bg-gray-50">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
            <ol class="flex flex-wrap items-center gap-1.5 text-sm text-gray-500">
                ...
                <li><Link href="/shop" class="hover:text-primary-600">{{ product.category }}</Link></li>
                <li aria-hidden="true" class="text-gray-300">/</li>
                <li aria-current="page" class="font-medium text-gray-900 max-w-[200px] sm:max-w-none truncate">
                    {{ product.name }}
                </li>
            </ol>
        </div>
    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:items-start lg:gap-12">
            ...
            <div>
                <!-- Main image -->
                <div class="overflow-hidden rounded-xl border border-gray-200 bg-gray-100 aspect-square">
                    <img
                ...
                <div class="mt-4 flex gap-3 overflow-x-auto no-scrollbar">
                    <button
                        ...
                        :class="[
                            ...
                            selectedImageIndex === idx ? 'border-primary-600 shadow' : 'border-transparent hover:border-gray-300'
                        ]"
            ...
            <div>
                <Link href="/shop" class="text-sm font-medium text-primary-600 hover:text-primary-700">
                    {{ product.category }}
                </Link>
                <h1 class="mt-2 text-2xl font-display font-extrabold tracking-tight text-gray-900 md:text-3xl">
                    {{ product.name }}
                </h1>
                ...
                <div class="mt-3 flex flex-wrap items-center gap-3">
                    <div class="flex items-center gap-0.5">
                        <svg
                            v-for="star in 5"
                            :key="star"
                            :class="['h-4 w-4', star <= Math.floor(product.rating) ? 'text-accent-500' : 'text-gray-300']"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                        >
                    ...
                    <button @click="switchTab('reviews')" class="text-sm text-gray-500 hover:text-primary-600 transition">
                        {{ product.reviews }} reviews
                    </button>
                    <span class="text-gray-300">|</span>
                    <span class="bg-green-50 text-green-700 text-xs font-medium px-2.5 py-1 rounded-full shadow-sm">
                        In Stock
                    </span>
                </div>
                ...
                <div class="mt-5 flex flex-wrap items-end gap-3">
                    ...
                    <span class="rounded-full bg-accent-600 px-2.5 py-1 text-xs font-semibold text-white shadow-sm">
                ...
                <div>
                    ...
                    <div class="inline-flex items-center rounded-lg border border-gray-300 bg-white">
                        <button
                            ...
                            :disabled="qty <= 1"
                            class="inline-flex h-11 w-11 items-center justify-center text-gray-600 hover:bg-gray-100 disabled:opacity-40 focus:outline-none rounded-l-lg"
                        >
                        ...
                        <button
                            ...
                            class="inline-flex h-11 w-11 items-center justify-center text-gray-600 hover:bg-gray-100 focus:outline-none rounded-r-lg"
                        >
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div ref="mainActionsRef" class="mt-5 flex flex-col gap-3 sm:flex-row">
                    <button
                        @click="handleAddToCart"
                        type="button"
                        class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-primary-600 px-5 py-3 text-sm font-semibold text-primary-600 hover:bg-primary-50 focus:outline-none focus:ring-2 focus:ring-primary-600 transition"
                    >
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <button
                        @click="handleBuyNow"
                        type="button"
                        class="inline-flex flex-1 items-center justify-center rounded-lg bg-gradient-to-r from-primary-600 to-primary-700 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:shadow-md hover:from-primary-700 hover:to-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-600 transition"
                    >
                        Buy Now
                    </button>
                    <button
                        ...
                        :class="[
                            'inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border transition focus:outline-none focus:ring-2 focus:ring-primary-600',
                            hasWish(product.name) ? 'border-red-600 text-red-600' : 'border-gray-300 text-gray-600 hover:border-red-300 hover:text-red-600'
                        ]"
                    >
                        <svg class="h-5 w-5" :fill="hasWish(product.name) ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="mt-6 space-y-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
                    <div class="flex items-start gap-3">
                        <svg class="mt-0.5 h-5 w-5 shrink-0 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <p class="text-sm text-gray-600"><span class="font-medium text-gray-900">Delivery:</span> ৳ 60 inside Dhaka, ৳ 120 outside Dhaka. Delivered in 2–4 days.</p>
                    </div>
                    <div class="flex items-start gap-3">
                        <svg class="mt-0.5 h-5 w-5 shrink-0 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <p class="text-sm text-gray-600"><span class="font-medium text-gray-900">Cash on Delivery</span> available — pay when you receive.</p>
                    </div>
                    <div class="flex items-start gap-3">
                        <svg class="mt-0.5 h-5 w-5 shrink-0 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                        </svg>
                        <p class="text-sm text-gray-600">Secure online payment via <span class="font-medium text-gray-900">SSLCommerz</span>.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="border-t border-gray-200 bg-gray-50">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 md:py-14">
            ...
            <div class="border-b border-gray-200">
                <div class="flex gap-6" role="tablist" aria-label="Product details">
                    <button
                        ...
                        :class="[activeTab === 'desc' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-900']"
                    >
                        Description
                    </button>
                    <button
                        ...
                        class="tab-btn -mb-px border-b-2 px-1 pb-3 text-sm font-semibold transition"
                        :class="[activeTab === 'reviews' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-900']"
                    >
                        Reviews <span class="text-gray-400">({{ reviews.length }})</span>
                    </button>
                </div>
            </div>
            ...
            <div v-if="activeTab === 'desc'" role="tabpanel" class="pt-6">
                <div class="max-w-prose space-y-4 text-sm leading-relaxed text-gray-600 md:text-base">
                    <p>The ShopEase Wireless Noise-Cancelling Headphones are engineered for listeners who refuse to compromise. Advanced hybrid active noise cancellation silences the world around you, while custom 40mm drivers deliver deep, balanced sound across every genre.</p>
                    <p>Designed for all-day comfort, the breathable memory-foam ear cushions and lightweight headband let you wear them from your morning commute to late-night sessions. With up to 40 hours of playback and fast charging — 10 minutes gives you 5 hours — you'll rarely reach for the cable.</p>
                    <h3 class="pt-2 text-base font-semibold text-gray-900">Key Features</h3>
                    <ul class="list-disc space-y-1.5 pl-5">
                        <li>Hybrid Active Noise Cancellation (ANC) with transparency mode</li>
                        <li>Up to 40 hours battery life; USB-C fast charging</li>
                        <li>Bluetooth 5.3 with multipoint pairing</li>
                        <li>Built-in microphone with environmental noise reduction for calls</li>
                ...
                <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <div class="lg:col-span-1">
                        <div class="rounded-xl border border-gray-200 bg-white p-6 text-center shadow-sm">
                            ...
                            <p class="mt-2 text-sm text-gray-500">Based on {{ reviews.length }} reviews</p>
                        </div>

                        <!-- Bar breakdown -->
                        <div class="mt-4 space-y-2">
                            <div v-for="b in breakdown" :key="b.s" class="flex items-center gap-2 text-sm">
                                <span class="w-8 shrink-0 text-gray-600">{{ b.s }} ★</span>
                                <div class="h-2 flex-1 overflow-hidden rounded-full bg-gray-200">
                                    <div class="h-full rounded-full bg-accent-500" :style="{ width: `${b.p}%` }"></div>
                                </div>
                                <span class="w-9 shrink-0 text-right text-gray-400">{{ b.p }}%</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-2 space-y-8">
                            ...
                            >
                                <div class="flex items-center justify-between gap-3">
                                    <div class="flex items-center gap-3">
                                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-primary-50 text-sm font-semibold text-primary-600">
                                            {{ r.name.charAt(0) }}
                                        </span>
                                        <div>
                                            <p class="text-sm font-semibold text-gray-900">
                                                {{ r.name }}
                                                <span v-if="r.verified" class="ml-1 rounded bg-green-50 px-1.5 py-0.5 text-[10px] font-medium text-green-700 align-middle shadow-sm">
                                                    Verified
                                                </span>
                                            </p>
                                            <div class="mt-0.5 flex gap-0.5">
                                                <svg
                                                    v-for="star in 5"
                                                    :key="star"
                                                    :class="['h-3.5 w-3.5', star <= r.rating ? 'text-accent-500' : 'text-gray-300']"
                                                    fill="currentColor"
                                                    viewBox="0 0 20 20"
                                                >
                                                    <path d="M9.05 2.927c.3-.921 1.6-.921 1.9 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.368 2.447a1 1 0 00-.364 1.118l1.287 3.957c.3.922-.755 1.688-1.539 1.118l-3.366-2.447a1 1 0 00-1.176 0l-3.366 2.447c-.783.57-1.838-.196-1.539-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.354 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.951-.69l1.286-3.957z" />
                                ...
                                <p class="mt-3 text-sm leading-relaxed text-gray-600">{{ r.text }}</p>
                            </div>
                        </div>
                        ...
                        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Your review will appear after approval by our team.</p>
                            <form @submit.prevent="handleReviewSubmit" class="mt-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-1" role="radiogroup" aria-label="Your rating">
                                        <button
                                            ...
                                            class="p-0.5 focus:outline-none focus:ring-2 focus:ring-primary-600 rounded"
                                        >
                                            <svg
                                                :class="['h-7 w-7 transition', star <= (hoverRating || selectedRating) ? 'text-accent-500 scale-110' : 'text-gray-300']"
                                                fill="currentColor"
                                                viewBox="0 0 20 20"
                                            >
                                                <path d="M9.05 2.927c.3-.921 1.6-.921 1.9 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.368 2.447a1 1 0 00-.364 1.118l1.287 3.957c.3.922-.755 1.688-1.539 1.118l-3.366-2.447a1 1 0 00-1.176 0l-3.366 2.447c-.783.57-1.838-.196-1.539-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.354 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.951-.69l1.286-3.957z" />
                                ...
                                <div>
                                    <label for="reviewName" class="mb-1.5 block text-sm font-medium text-gray-700">Name</label>
                                    <input
                                        id="reviewName"
                                        v-model="reviewName"
                                        type="text"
                                        required
                                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-primary-600 focus:ring-2 focus:ring-primary-600 focus:outline-none transition bg-white"
                                        placeholder="Your name"
                                    />
                                </div>
                                <div>
                                    <label for="reviewText" class="mb-1.5 block text-sm font-medium text-gray-700">Review</label>
                                    <textarea
                                        id="reviewText"
                                        v-model="reviewText"
                                        rows="4"
                                ...
                                <button
                                    type="submit"
                                    class="rounded-lg bg-primary-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-600 transition shadow-sm"
                                >
                                    Submit Review
                                </button>
                            </form>
    ...
    <section class="bg-white py-12 md:py-16">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 class="mb-8 text-2xl font-display font-extrabold tracking-tight text-gray-900 md:text-3xl">You may also like</h2>
            <div class="grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-6 lg:grid-cols-4">
                ...
                >
                    <div class="relative aspect-square overflow-hidden bg-gray-100">
                        ...
                        <button
                            @click="toggleWish(p.name); showToast(hasWish(p.name) ? 'Added to wishlist ❤️' : 'Removed from wishlist')"
                            type="button"
                            aria-label="Add to wishlist"
                    ...
                    <div class="flex flex-1 flex-col p-3 md:p-4">
                        ...
                        <div class="mt-auto pt-3">
                            <div class="flex flex-col gap-2">
                                <button
                                    v-if="p.inStock"
                                    @click="addToCart(p.name, p.price, p.img); showToast(`Added to cart: ${p.name} 🛒`)"
                                    type="button"
                                ...
                                <button
                                    v-if="p.inStock"
                                    @click="buyNow(p.name, p.price, p.img); showToast(`Added to cart: ${p.name} 🛒`)"
                                    type="button"
    ...
    <Transition name="fade-slide">
        <div v-show="showStickyBar" class="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white/95 backdrop-blur lg:hidden shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
            <div class="flex items-center gap-3 px-4 py-3">
                <div class="leading-tight shrink-0">
                    <p class="text-lg font-bold text-primary-600">{{ formatPrice(product.price) }}</p>
                    <p v-if="product.oldPrice" class="text-xs text-gray-400 line-through">{{ formatPrice(product.oldPrice) }}</p>
                </div>
                <button
                    @click="handleAddToCart"
                    type="button"
                    class="flex-1 rounded-lg border border-primary-600 px-4 py-3 text-sm font-semibold text-primary-600 hover:bg-primary-50 transition"
                >
                    Add to Cart
                </button>
                <button
                    @click="handleBuyNow"
                    type="button"
                    class="flex-1 rounded-lg bg-gradient-to-r from-primary-600 to-primary-700 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:shadow-md hover:from-primary-700 hover:to-primary-700 transition"
                >
                    Buy Now
                </button>
            </div>
